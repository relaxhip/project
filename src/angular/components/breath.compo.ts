...
@Component({
	selector: 'breath-effect',
	templateUrl: './components/effect/breath.compo.html',
	//template: '<h1>我的第一个 Angular 应用</h1>',
	styleUrls: ['./css/first.css', './css/kbd.css', './css/breath.css', './css/attractlight.css'],
...
export class breathComponent implements OnInit {
	constructor(private protocol: protocolService, private db: dbService, private getDeviceService: GetDeviceService, private emitService: EmitService, private icpEventService: icpEventService) {
		//console.log('breath loading complete');
	...
	ngOnInit() {       //呼吸
		console.log(this.receiveTemp)
		this.timeValue = this.receiveTemp[3];
		this.check01 = false;
		this.BreathTemp.emit(this.breathT);
		env.log('light-effect', 'breath', 'start');
	...
	ngOnChanges(changes: any) {
		...
		if (this.currentV == '"stopApmode"' && this.savechange == 0) {
			...
			this.goOut();
			// if (this.envio == 1) {
				env.log('breath', 'apmode', 'stopapmode')
			// }
			// this.savechange = 0
			//console.log('stopapmopde222')
			// //console.log(this.savechange)
		}
		if (this.currentV == '"startApmode"' && this.savechange == 0) {
			...
			this.attBar(0);
			// if (this.envio == 1) {
				env.log('breath', 'apmode', 'startapmode')
			// }
			// this.savechange = 0
			//console.log('333')
			// //console.log(this.savechange)
	...
	//
	thelastEffect: any = this.breath_med;
	...
	breath_high() {       //呼吸   
		...
		this.thelastEffect = this.breath_high;
		this.breath = "breath_high";
	}
	breath_med() {       //呼吸  
		this.getColor1();
		this.getPt = this.attPt01;
		// this.getColor = this.getColor1;
		this.thelastEffect = this.breath_med;//儲存為最後一個動畫效果
		this.breath = "breath_med"
		//加入具有css動畫的class名稱
	}
	breath_slow() {       //呼吸   
		this.getColor1();
		this.getPt = this.attPt01;
		// this.getColor = this.getColor1;
		this.thelastEffect = this.breath_slow;
		this.breath = "breath_slow"
	}
	...
	slowFun: any = this.breath_slow;
	medFun: any = this.breath_med;
	fastFun: any = this.breath_high;
	speed: any = '6';

	// //最先決定速度類型

	speedMethod(s) {
		clearInterval(this.getclr01);  //選擇速度變換效果前關閉取值
	...
	getRule() {
		...
		for (var i = 0; i < ss.length; i++) {
			for (var x = 0; x < ss[i]['cssRules'].length; x++) {
				...
				if (rule.name == "breath" && rule.type == rule.KEYFRAMES_RULE) {
					this.cssRule = rule;
				}
			}
		}
		// this.cssRule.appendRule(`25% {background-color:${this.color1[Math.floor(Math.random()*7)]}}`);
		// this.cssRule.appendRule(`75% {background-color:${this.color2[Math.floor(Math.random()*7)]}}`);
		this.cssRule.appendRule(`25% {background-color:${this.defaultclr}}`);
		this.cssRule.appendRule(`75% {background-color:${this.defaultclr02}}`);
		this.detectEffectChange();
	}
	//
	...
	getRuleRandom() {
		...
		this.cssRule.appendRule(`25% {background-color:${this.color1[ran1]}}`);
		this.cssRule.appendRule(`75% {background-color:${this.color2[ran2]}}`);
		this.detectEffectChange();
	}
	...
	setIntoDB() {
		...
		// this.nowobj.Light.LightSetting.LSspeed[1] = this.sp;
		// //console.log(this.nowobj.Light.LightSetting.LSspeed)


		this.nowobj.Light.LightSetting.LSbrightness[1] = this.op;
		this.nowobj.Light.LightSetting.LSspeed[1] = this.sp;
		// this.nowobj.Light.LightSetting.LSdirection[0] = this.Effectdirection;
		this.nowobj.Light.LightSetting.changeTime[1] = this.timeValue;

		// //console.log(this.timeValue);
		this.nowobj.Light.LightSetting.changeMode[1] = this.ttitle;
	...
	ngOnDestroy(): void {
		this.subscription.unsubscribe();
		this.goOut();
		env.log('light-effect', 'breath', 'end');
	}
	...
	@Input() timeEffect;
	@Input() receiveTemp;
	@Output() BreathTemp = new EventEmitter();
	...
	breathObj() {
		...
		// this.BreathTemp.emit(this.breathT);
		// this.LightEffect.emit(this.breathT.LightEffect)
	}
	readTemp() {
